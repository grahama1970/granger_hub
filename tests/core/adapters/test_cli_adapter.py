...
class TestCLIAdapterReal:
    ...
    async def test_real_process_execution(self):
        ...
        async def test_real_python_execution(self):
            ...
            adapter = CLIAdapter(config, command="python3")
            ...
            # Create a custom command that doesn't use the args processing
            adapter.command = ["python3", "-c", "import time; time.sleep(0.1); print('Python executed:', 42)"]
            result = await adapter.send({})
                        ...
                        for i in range(3):
                            ...
                            for adapter in adapters:
                                ...
                                for i, adapter in enumerate(adapters):
                                    # Set the full command to avoid args processing issues
                                    adapter.command = ["python3", "-c", f"import time; time.sleep(0.05); print('Process {i}')"]
                                    task = adapter.send({})
                                    tasks.append(task)
